...
import pytest
from malefico.apis.multiprocessing import AsyncResult, Pool, Queue
from malefico.core.messages import Cpus, Disk, Mem
from malefico.utils import TimeoutError
...
def test_queue_apply_async(zk, resources):
    ...
    queue = Queue(zk, '/malefico/test-pool')
    with Pool(name='test-pool') as pool:
