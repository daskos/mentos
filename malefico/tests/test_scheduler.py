...
import pytest
from malefico.constraint import has
from malefico.messages import PythonExecutor, PythonTask, PythonTaskStatus
from malefico.core.messages import (Cpus, Disk, Mem, Offer, OfferID, SlaveID,Environment,
                                    TaskID)
...
def test_scheduler_constraints(mocker):
    ...
    with MesosSchedulerDriver(sched, name='test-scheduler') as driver:
        sched.submit(task)
        sched.wait()
